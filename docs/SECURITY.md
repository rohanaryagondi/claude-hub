...
`claude-hub` reads data from the active Claude Code config directory. By default this is `~/.claude/`; users can override it with `CLAUDE_CONFIG_DIR`.
...
- Export creates a downloaded `.claude-hub.json` file when the user requests it.
