...
By default, `claude-hub` reads from `~/.claude/`. You can point it at another profile with `CLAUDE_CONFIG_DIR`.
...
Nothing is intentionally uploaded by `claude-hub`.
...
The export feature can generate a portable `.claude-hub.json` file when you request it. Treat this file as private because it may contain session metadata, facets, and command history.
