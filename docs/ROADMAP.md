...
This roadmap is intentionally practical. `claude-hub` should stay local-first, fast to launch, and useful without an account or hosted backend.

## Near Term

- **Demo mode**: Add `npx claude-hub --demo` with anonymized sample data so users can evaluate the dashboard before they have Claude Code history.
