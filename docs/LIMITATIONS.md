...
`claude-hub` reads local Claude Code files directly. That makes it private and useful, but it also means the app depends on file formats that can change.
...
- Export files can contain sensitive local workflow data. Treat `.claude-hub.json` files as private.
...
- If Claude Code changes its local file layout, some panels may show partial data until `claude-hub` is updated.
