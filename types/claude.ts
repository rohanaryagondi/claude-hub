...
export interface DailyActivity {
  ...
  tokenCount?: number  // input + output tokens only (excludes cache)
}
...
export interface ProjectSummary {
  ...
  uses_task_agent: boolean
  branches: string[]
  last_prompt?: string      // first_prompt of the most recent session
  activity?: number[]       // session count per day for the last 14 days (oldest → newest)
  recap?: string            // Claude-written status (memory layer): project recap, or the latest session's recap when fresher
  status?: string           // memory layer: one-line current state (what works, where it stands)
  decisions?: string[]      // memory layer: notable choices/decisions made (up to 5)
}
