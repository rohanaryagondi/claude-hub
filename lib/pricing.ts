...
// Loads ~/.claude-hub/pricing.json if present. Server-side only; cached for
...
function loadUserOverrides(): Record<string, ModelPricing> {
  ...
  try {
    ...
    const configDir = process.env.CLAUDE_HUB_CONFIG_DIR ?? path.join(os.homedir(), '.claude-hub')
    ...
    for (const [model, entry] of Object.entries(raw)) {
      if (isValidEntry(entry)) {
        ...
        console.warn(`[claude-hub] pricing.json: skipping invalid entry for "${model}"`)
      }
    }
    return out
  } catch (err) {
    console.warn('[claude-hub] failed to load pricing.json:', (err as Error).message)
    return {}
  }
}
